...
def handle_macro(json_str, **kwargs):
    temp_dict = json.loads(json_str, **kwargs)
    temp_str = json_str
    if type(temp_dict["$MACRO$"]) is not dict:
        raise Exception("$MACRO$ property should be dict(json obj) but not json list.")
    for _macro_name, _macro_val in temp_dict["$MACRO$"].items():
        temp_str = temp_str.replace(
            ('"$'+_macro_name+'$"') if type(_macro_val) is int else ("$"+_macro_name+"$"),
            str(_macro_val))
    return json.loads(temp_str, **kwargs)
