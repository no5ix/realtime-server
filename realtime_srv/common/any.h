...
// N4562 std::experimental::any (merged into C++17) for C++11 compilers.
// 
// It contains a small object optimization for objects with
// a size of up to 2 words(such as int, float and std::shared_ptr).
// Storing those objects in the container will not trigger a dynamic allocation.
//
//
// # Advanced
//
// You may additionally define the following preprocessor 
// symbols(making the implementation non - standard) :
// 
// ANY_IMPL_ANY_CAST_MOVEABLE: 
// 	This implements a fix proposed in LWG Defect 2509.
// 	This will cause the expressions T x = any_cast<T>(any(T())) 
// 	and
// 	T x = any_cast<T&&>(any(T()))to perform a move into x instead of a copy.
// 
// ANY_IMPL_FAST_TYPE_INFO_COMPARE :
//	When checking if two typeid are the same,
//	performs just a pointer comparision instead of the actual
//		type_info::operator== comparision.
//	Be aware this isn't recommended unless you know what you're doing.
//
//
//
...
namespace std
{
inline void swap(realtime_srv::any& lhs, realtime_srv::any& rhs) noexcept
{
	lhs.swap(rhs);
}
